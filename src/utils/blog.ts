...
// Tracked redirect links generated by the admin point at the backend's
// /affiliate-links/{id}/go endpoint (which 302s to the real affiliate URL).
// Their host is our own API domain, so the host allowlist above won't catch
// them — match the path instead so they're still marked sponsored.
const TRACKED_AFFILIATE_PATH = /\/affiliate-links\/[^/]+\/go\/?$/;

export function isTrackedAffiliateUrl(url: string): boolean {
    try {
        return TRACKED_AFFILIATE_PATH.test(new URL(url, 'https://luci-studio.com').pathname);
    } catch { return false; }
}

// rel for an outbound markdown link. Affiliate links (direct or tracked-redirect)
// additionally get "sponsored nofollow"; everything else keeps the safety pair.
function linkRel(url: string): string {
    return isAffiliateUrl(url) || isTrackedAffiliateUrl(url)
